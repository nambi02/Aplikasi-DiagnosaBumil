 ...
 * @author mrssy
 */
public class FormKonsultasi2 extends javax.swing.JInternalFrame {
    ...
    private void simpanDiagnosis() {
     try {
        ...
        String query = "INSERT INTO diagnosa (idkonsultasi, tanggalkonsul, id, nama_pasien, umur_pasien, notelp, alamat, kodep, namap, deskripp, saranp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " +
                       "ON DUPLICATE KEY UPDATE tanggalkonsul = VALUES(tanggalkonsul), id = VALUES(id), nama_pasien = VALUES(nama_pasien), umur_pasien = VALUES(umur_pasien), notelp = VALUES(notelp), " +
                       "alamat = VALUES(alamat), kodep = VALUES(kodep), namap = VALUES(namap), deskripp = VALUES(deskripp), saranp = VALUES(saranp)";
        PreparedStatement ps = con.prepareStatement(query);
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                            ...
                            .addComponent(bcari1, javax.swing.GroupLayout.PREFERRED_SIZE, 28, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
 ...
 private String getKodePenyakit(List<String> gejalaList) {
    ...
    try {
        if (gejalaList.size() > 3) { // Memastikan lebih dari 3 gejala yang dipilih
            Connection con = DriverManager.getConnection(db, user, password);
            ...
            if (rs.next()) {
                String resultKodePenyakit = rs.getString("kodep");
                
                // Memastikan kode penyakit hanya dikembalikan jika jumlah gejala yang dipilih sesuai dengan rule
                if (isMatchingAllGejala(gejalaList, resultKodePenyakit)) {
                    kodePenyakit = resultKodePenyakit;
                }
            }
...
private boolean isMatchingAllGejala(List<String> gejalaList, String kodePenyakit) {
    boolean isMatching = false;
    try {
        Connection con = DriverManager.getConnection(db, user, password);
        
        // Query untuk memeriksa gejala lengkap yang terkait dengan kode penyakit
        String query = "SELECT kodeg FROM gejalapenyakit WHERE kodep = ?";
        PreparedStatement ps = con.prepareStatement(query);
        ps.setString(1, kodePenyakit);
        ...
        if (rs.next()) {
            String[] requiredGejala = rs.getString("kodeg").split(",");
            List<String> requiredGejalaList = Arrays.asList(requiredGejala);

            // Memeriksa apakah semua gejala yang diperlukan ada dalam gejala yang dipilih
            if (gejalaList.containsAll(requiredGejalaList)) {
                isMatching = true;
            }
        }
